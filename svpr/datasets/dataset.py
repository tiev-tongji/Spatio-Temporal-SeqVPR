...
from svpr.utils import RAMEfficient2DMatrix
...
class BaseDataset(data.Dataset):
    def __init__(self, cities='', dataset_folder="datasets", split="train", base_transform=None,
        ...
        self.database_utms = np.array(
            [(path.split("@")[1], path.split("@")[2]) for path in all_db_paths[db_unique_idxs]]).astype(np.float32)
        self.queries_utms = np.array(
            [(path.split("@")[1], path.split("@")[2]) for path in all_q_paths[q_unique_idxs]]).astype(np.float32)
        ...
        for q in tqdm(range(len(q_idx_frame_to_seq)), ncols=100, desc='Finding positives and negatives...'):
            ...
            p_uniq_frame_idxs = np.unique(
                [pos for pos in self.hard_positives_per_query[unique_q_frame_idxs[0][self.seq_len//2]]])

            if len(p_uniq_frame_idxs) > 0:
                p_seq_idx = np.where(np.in1d(db_idx_frame_to_seq, db_unique_idxs[p_uniq_frame_idxs]).reshape(db_idx_frame_to_seq.shape))
                p_seq_idx = p_seq_idx[0][np.where(p_seq_idx[1]==2)]
                
                if len(p_seq_idx) == 0:
                    self.q_without_pos += 1
                    continue
                
                self.valid_q.append(self.q_paths[q])
                self.qIdx.append(q)
    ...
    def __getitem__(self, index):
        ...
        if self.crop:
            img = torch.stack([self.base_transform(Image.open(im).crop((0,0,1280,800))) for im in self.images_paths[index].split(',')])
        else:
...
class TrainDataset(BaseDataset):
    def __init__(self, cities='', dataset_folder="datasets", split="train", base_transform=None,
        ...
        self.n_gpus = n_gpus
        self.num_workers = 20 * int(n_gpus)
    ...
    def __getitem__(self, index):
        ...
        if self.crop:
            query = torch.stack(
            [self.base_transform(Image.open(im).crop((0,0,1280,800))) for im in self.q_paths[query_index].split(',')])

            positive = torch.stack(
                [self.base_transform(Image.open(im).crop((0,0,1280,800))) for im in self.db_paths[best_positive_index].split(',')])

            negatives = [torch.stack([self.base_transform(Image.open(im).crop((0,0,1280,800)))for im in self.db_paths[idx].split(',')])
    ...
    def compute_triplets(self, model, args):
        self.is_inference = True
        self.compute_triplets_partial(model, args)
        self.is_inference = False

    def compute_cache(self, model, args, subset_ds, cache_shape):
    ...
    def compute_triplets_partial(self, model, args):
        ...
        cache = self.compute_cache(model, args, subset_ds, cache_shape=(len(self), self.features_dim))
